...
//        listAdd();
    ...
    private void regRequest() {
        Outbox.ReqOutBox index = Outbox.ReqOutBox.newBuilder().setSid(User.sid).setId(xqid).setAc("FZLXQ").build();
        new HttpUtilO() {
            @Override
...
//                xiangqingCon2.setText(index.getPa().getCon());
                ...
                ja = new CommonAdapter<Outbox.ReqOutBox.FileList>(XieYouJianActivity.this, arrayList, R.layout.xia_zai_item) {
                    ...
                    public void convert(ViewHolder holder, Outbox.ReqOutBox.FileList ceshi) {
                        ...
                        if (SharedPreferencesUtils.contains(XieYouJianActivity.this,ceshi.getFpk())) {
...
//                        Log.i("zhang", "sd卡的路径：" + Environment.getExternalStorageDirectory().getPath()+"/"+filename);
                        ...
                        XiaZaiUtil xiaZaiUtil = new XiaZaiUtil(holder.<ImageButton>getView(R.id.xiazai_iamge_button), XieYouJianActivity.this, ceshi.getUrl(), holder.<MasterLayout>getView(R.id.xiazai_zx), holder.<TextView>getView(R.id.xiazai_sudu),ceshi.getName(),ceshi.getFpk());
    ...
    public void onClick(View view) {
        switch (view.getId()) {
            case R.id.xieyoujian_shenchu:
                Outbox.ReqOutBox index = Outbox.ReqOutBox.newBuilder().setSid(User.sid).setId(xqid).setAc("DLFZL").build();
                new HttpUtilO() {
                    @Override
                    public <T> void analysisInputStreamData(Outbox.ReqOutBox index) throws IOException {
                ...
                }.protocolBuffer(XieYouJianActivity.this, Globals.ZL_URI, index, null);
                break;
